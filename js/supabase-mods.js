...
// ============================================================
// Uses the shared client from js/supabase-client.js — put your
// project URL + anon key there (not in this file).
// See SETUP.md for the `mods` table schema.
// ============================================================
...
// ---------- card variants ----------
// Every card links to mod.html?id=<row id> for a full detail page.
// variant: "home"     -> thumb + tags + title + desc + status hint
...
export function renderModCard(mod, variant = "home") {
  ...
  if (variant === "home") {
    const hint = mod.status === "released"
      ? `<span class="card-dl" style="display:flex;align-items:center;justify-content:center;">Download</span>`
      : `<span class="card-dl" style="cursor:pointer;">Coming soon</span>`;
    return `
...
// ---------- fetch a single mod by id (for mod.html) ----------
export async function fetchModById(id) {
  try {
    const { data, error } = await supabase.from("mods").select("*").eq("id", id).single();
    if (error) {
      console.error("Supabase returned an error fetching mod:", error);
